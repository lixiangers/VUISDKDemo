...
public class CustomAndroidAudioGenerator implements BaseAudioGenerator {
    ...
    private static final String TAG = "CustomAndroidAudioGenerator";
...
//        timer.check("[getNextFrame]");
    ...
    private Runnable mReadTask = new Runnable() {
        ...
        @Override
        public void run() {
            Log.d(TAG, "RooboRecorder begin recording... + mMinBufferSize=" + mRecBufSize);
            ...
            while (!mFinished) {
                ...
                if (len <= 0) {
                    Log.e(TAG, "read error,lastWriteRemain:" + lastWriteRemain + " len:" + len);
                    continue;
                }
                int pos = 0;
